...
export default function RecruitDetailClient({ recruit }: RecruitDetailClientProps) {
  ...
  const handleApply = async () => {
    ...
    setIsApplying(true);
    try {
      await applyRecruit(recruit.id, { reason: '' }, { accessToken: session.accessToken });
      ...
      {isOpen && (
        <div className="mt-4">
          <button
            onClick={handleApply}
            disabled={isApplying}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-gray-900 py-3.5 text-sm font-medium text-white transition hover:bg-gray-800 disabled:bg-gray-300"
          >
            {isApplying ? (
              <>
