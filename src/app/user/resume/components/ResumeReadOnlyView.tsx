 ...
 */
export default function ResumeReadOnlyView({ data, profileImageUrl, resumeTitle }: Props) {
  return (
              ...
              <div key={proj.id ?? i}>
                ...
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm font-semibold text-gray-900">
                    {proj.name || '-'}
                    {proj.featured === 'true' && (
                      <span className="ml-2 text-xs text-orange-500">★ 대표</span>
                    )}
                  </p>
                ...
                {proj.techStack && (
                  <p className="mt-0.5 text-xs text-gray-500">사용 기술: {proj.techStack}</p>
                )}
                {proj.summary && (
                  <p className="mt-2 text-sm text-gray-700 whitespace-pre-wrap">{proj.summary}</p>
                )}
                {proj.mainFeatures && (
                  <div className="mt-2">
          ...
          <div className="space-y-4">
            {data.awards.map((award, i) => (
              <div key={award.id ?? i}>
                ...
                {award.organization && (
                  <p className="mt-0.5 text-xs text-gray-500">{award.organization}</p>
                )}
                {award.description && (
                ...
                <div className="text-right shrink-0 ml-4">
                  ...
                  {cert.status && (
                    <span className={`text-xs font-medium ${
                      cert.status === 'ACQUIRED' ? 'text-emerald-600' : 'text-gray-400'
                    }`}>
          ...
          <div className="space-y-6">
            {data.coverLetters.map((cl, i) => (
              <div key={cl.id ?? i}>
                {i > 0 && <Divider />}
                {cl.title && (
                  <p className="mb-2 text-sm font-semibold text-gray-900">{cl.title}</p>
                )}
